...
export default function Post({ post }) {
  ...
  let userImage =
    post?.userId?.profilePicture.length > 0
      ? endPoint + post?.userId?.profilePicture
  ...
  function handleLikeDislike(type) {
    if (type === "like") {
      axios
        .put(`http://localhost:8800/api/post/${post?._id}/like`, {
          userId: loggedInUser?._id,
        })
        .then((res) => dispatch({ type: "LIKE", payload: res.data }));
    } else {
      axios
        .put(`http://localhost:8800/api/post/${post?._id}/dislike`, {
          userId: loggedInUser?._id,
        })
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8800/api/comment/get-comments/${post?._id}`)
      .then((res) => {
  ...
  async function addComment(comment, id = null) {
    ...
    if (id != null) {
      response = await axios.post(
        `http://localhost:8800/api/comment/reply/${id}`,
        {
          userId: loggedInUser?._id,
          postId: post?._id,
          comment: comment,
      ...
      response = await axios.post(
        "http://localhost:8800/api/comment/add-comment",
        {
          userId: loggedInUser?._id,
          postId: post?._id,
          comment: comment,
